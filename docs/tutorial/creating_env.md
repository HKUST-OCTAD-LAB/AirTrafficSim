...
```py title="airtrafficsim_data/environment/DemoEnv.py"
...
Air traffic is stored in arrays at `Traffic` class in [airtrafficsim.core.traffic.Traffic][]. Each `Aircraft` in [airtrafficsim.core.aircraft.Aircraft][] represents an individual aircraft in the traffic array and provides an interface to command the aircraft movement. The parameters to initialize an aircraft are explained below.
...
|   flight_phase    |       [Current flight phase][airtrafficsim.utils.enums.FlightPhase]        |
|   configuration   |         [Current configuration][airtrafficsim.utils.enums.Config]          |
|   **Position:**   |
