...
import {
    IonContent, IonPage, IonTitle, IonToolbar, IonRange, IonIcon,
    IonButtons, IonButton, IonProgressBar, IonLabel, IonItem,
    IonSelect, IonSelectOption, IonGrid, IonCol, IonRow, IonFooter,
    IonChip, IonModal, IonToggle, IonList, useIonViewDidEnter, IonHeader,
    IonSegment, IonSegmentButton, IonLoading, IonToast
} from '@ionic/react';
import {
    stop, stopOutline,
...
const SimulationOld: React.FC = () => {
    ...
    function getNav(selected: boolean, value: boolean) {
        let get = selected ? value : showNavigationData;
    ...
    function getEra5Wind(selected: boolean, value: boolean) {
        let get = selected ? value : era5Wind;
        if (get && viewerRef.current?.cesiumElement) {
            const viewer = viewerRef.current.cesiumElement;
    ...
    function getRadarImg(selected: boolean, value: boolean) {
        ...
        if (get && viewerRef.current?.cesiumElement) {
            ...
            socket.emit(
                ...
                mode === 'replay' ? replayFile : simulationFile,
                julianDate.replace('Z', ''),
                (res: any) => {
                    console.log(res);
    ...
    return (
        <IonPage>
            <IonContent scrollY={false}>
                <Viewer ref={viewerRef} style={{ height: "100%" }} animation={false} timeline={false} selectionIndicator={false} homeButton={false} baseLayerPicker={false} sceneModePicker={false} fullscreenButton={false} navigationHelpButton={false} geocoder={false} >
                    <Scene debugShowFramesPerSecond={true} />
                    <Globe
                        baseColor={Color.fromCssColorString('#000000')}
                        terrainProvider={Ion.defaultAccessToken ? terrainProvider : undefined}
                        showGroundAtmosphere={false}
                    />
                    {/* <ImageryLayer imageryProvider={simpleImagery} /> */}
                ...
                <IonToolbar>
                    <IonGrid fixed style={{ "--ion-grid-padding": "0px", "--ion-grid-column-padding": "0px", "--ion-grid-width-xl": "90%", "--ion-grid-width-lg": "100%", "--ion-grid-width-md": "100%", "--ion-grid-width-sm": "100%", "--ion-grid-width-xs": "100%" }} >
                        <IonRow className="ion-align-items-center ion-justify-content-center">
                            <IonCol size="auto">
                                <IonItem className="ion-no-padding" lines="none" style={{ "width": "150px", "height": "100%", "--inner-padding-end": "0px", "--background": "transparent" }} href="https://github.com/HKUST-OCTAD-LAB/AirTrafficSim" target="_blank">
                                    <IonTitle>AirTrafficSim</IonTitle>
                                </IonItem>
                            </IonCol>
                            <IonCol size="auto">
                                <IonItem className="ion-nowrap ion-no-padding" lines="none" style={{ "width": "200px", "--inner-padding-end": "0px", "--background": "transparent" }}>
                                ...
                                <IonModal isOpen={showReplayModal} onDidDismiss={() => setShowReplayModal(false)}>
                                    <IonHeader translucent>
                                        ...
                                        <IonToolbar>
                                            <IonSegment color="dark" value={replayCategory.toString()} onIonChange={e => setReplayCategory(e.detail.value as string)}>
                                                ...
                                                {replayList[replayCategory].map((file: any) =>
                                                    <IonItem key={file} button={replayCategory === "historic" ? true : false}
                                                        ...
                                                        {replayCategory === "simulation" &&
                                                            <IonSelect color='dark' interface="alert" aria-label="Simulation File"
                            ...
                            <IonCol size="auto">
                                <IonItem className="ion-no-padding" lines="none" style={{ "width": "100px", "--inner-padding-end": "0px", "--background": "transparent" }} button={true} onClick={() => setShowSettingModal(true)}>
                                    <IonIcon icon={settings} />
                                    <IonLabel className="ion-text-center">Settings</IonLabel>
                                </IonItem>
                            ...
                            <IonCol size="auto">
                                <IonItem lines="none" style={{ "width": "100px", "--inner-padding-end": "0px", "--background": "transparent" }}>
                                    <IonLabel style={{ "whiteSpace": "pre-line" }} className="ion-text-center">{julianDate.replace("T", "\n")}</IonLabel>
                                </IonItem>
                            </IonCol>
                            <IonCol >
                                <IonItem lines="none" style={{ "--background": "transparent" }}>
                                    <IonRange min={0} max={endTime} value={clockTime} color='dark'
                                        ...
                                        aria-label={Math.trunc(clockTime) + "s"}
                                    />
                                </IonItem>
                            </IonCol>
                        </IonRow>
...
const Component1 = (props: {
    ...
    useEffect(() => {
        props.viewer.dataSources.add(replayDataSource);
    }, [])
...
const Simulation: React.FC = () => {
    const [cv, setCV] = useState<CesiumViewer | null>(null);
    
    const viewerRefCallback = useCallback(node => {
        if (node !== null && node.cesiumElement !== undefined) {
            setCV(node.cesiumElement);
            console.log("start")
        }
    }, [])

    return <>
        <Viewer ref={viewerRefCallback} style={{ height: "100%" }} animation={false} timeline={false} selectionIndicator={false} homeButton={false} baseLayerPicker={false} sceneModePicker={false} fullscreenButton={false} navigationHelpButton={false} geocoder={false} >
            <Globe
                baseColor={Color.fromCssColorString('#000000')}
