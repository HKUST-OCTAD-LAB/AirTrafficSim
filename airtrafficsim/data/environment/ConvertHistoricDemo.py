...
class ConvertHistoricDemo(Environment):
    def __init__(self) -> None:
        ...
        self.historic_data_path = (
            Path(__file__)
            .parent.parent.parent.resolve()
            .joinpath("data/flight_data/2018-05-01/")
        )

        print("Analyzing flight data")
        # Set up arrival and approach data
        arrivals_dict, arrival_waypoints_coord_dict = get_arrival_data(
            "VHHH", "07R"
        )
        approach_dict, approach_waypoints_coord_dict = get_approach_data(
            "VHHH", "07R"
        )
        ...
        for file in self.historic_data_path.iterdir():
            ...
            arrival_result, arrival_trajectory = detect_sid_star(
                simplified, arrivals_dict, arrival_waypoints_coord_dict
            )
            self.star.append(arrival_result)
            approach_result, approach_trajectory = detect_sid_star(
                simplified, approach_dict, approach_waypoints_coord_dict
            )
    ...
    def atc_command(self) -> None:
        ...
        for i in index:
            self.aircraft_list[self.call_sign[i]] = Aircraft(
                self.traffic,
                call_sign=self.call_sign[i],
                aircraft_type=self.type[i],
                flight_phase=FlightPhase.CRUISE,
                configuration=Config.CLEAN,
                lat=self.position[i][0],
                long=self.position[i][1],
                alt=self.start_alt[i],
