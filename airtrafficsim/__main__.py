...
def main():
    ...
    if (
        not Path(__file__)
        .parent.resolve()
        .joinpath("./data/client/build/")
        .is_dir()
    ):
        print("Unzipping client.")
        ZipFile(
            Path(__file__).parent.resolve().joinpath("./data/client/build.zip")
    ...
    if args.init:
        ...
        if Path.cwd().joinpath(args.init).is_dir():
            Path.cwd().joinpath(args.init).resolve().joinpath(
                "airtrafficsim_data"
            ).symlink_to(
                Path(__file__).parent.resolve().joinpath("./data"),
                target_is_directory=True,
            )
            print(
                "Created airtrafficsim_data at "
                + str(
                    Path.cwd()
                    .joinpath(args.init)
                    .resolve()
                    .joinpath("airtrafficsim_data")
                    .resolve()
                )
            )
        else:
        ...
        if (
            len(
                list(
                    Path(__file__)
                    .parent.resolve()
                    .joinpath("./data/performance/BADA/")
                    .glob("*")
                )
            )
            <= 1
        ...
        if args.headless:
            ...
            Env = getattr(
                import_module(
                    "airtrafficsim.data.environment." + sys.argv[2], "..."
                ),
                sys.argv[2],
            )
            env = Env()
