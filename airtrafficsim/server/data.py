...
class Data:
    ...
    def get_era5_wind(file, lat1, long1, lat2, long2, time):
        ...
        data = xr.open_dataset(Path(__file__).parent.parent.parent.joinpath('data/weather/era5/', file.split(
            '-', 1)[0]+'/multilevel.nc')).sel(level=900, time=datetime.fromisoformat(time), method="pad")
        data = data.where((((data.latitude >= lat1) & (data.latitude <= lat2)) & (
            (data.longitude >= (long1+360.0) % 360.0) & (data.longitude <= (long2+360.0) % 360.0))), drop=True)
        fig = Figure(figsize=(long2-long1, lat2-lat1),
                     facecolor='none', dpi=500)
        ax = fig.add_axes(
        ...
        uri = "data:image/png;base64," + \
            base64.b64encode(buf.getbuffer()).decode("ascii")
        return [
            {
                "id": "document",
                "name": "Weather",
    ...
    def get_era5_rain(file, lat1, long1, lat2, long2, time):
        ...
        """
        # TODO: Improve data loading to avoid repetitive loading
        data = xr.open_dataset(Path(__file__).parent.parent.parent.joinpath(
            'data/weather/era5/', file.split('-', 1)[0]+'/surface.nc')).sel(time=datetime.fromisoformat(time), method="pad")
        data = data.where((((data.latitude >= lat1) & (data.latitude <= lat2)) & (
            (data.longitude >= (long1+360.0) % 360.0) & (data.longitude <= (long2+360.0) % 360.0))), drop=True)
        fig = Figure(figsize=(long2-long1, lat2-lat1),
                     facecolor='none', dpi=500)
    ...
    def get_radar_img(file, lat1, long1, lat2, long2, time):
        ...
        for file in Path(__file__).parent.parent.parent.joinpath('data/weather/radar/', file.split('-', 1)[0]).iterdir():
            if (datetime.fromisoformat(time+'+00:00') - datetime.fromisoformat(file.stem+'+00:00')).seconds < 3600:
                ...
                distant = [np.square(data[:, :, 0].flatten() - rain_fall_r[i]) + np.square(data[:, :, 1].flatten(
