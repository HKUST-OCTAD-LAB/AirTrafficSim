...
class Data:
    ...
    def get_nav(lat1: float, long1: float, lat2: float, long2: float) -> Czml:
        ...
        fixes, navaids = Nav.get_wp_in_area(lat1, long1, lat2, long2)

        for fix in fixes:
            document.append(
                {
                    "id": fix[2],
                    "position": {"cartographicDegrees": [fix[1], fix[0], 0]},
                    "point": {
                        "pixelSize": 4,
                        "color": {"rgba": [39, 243, 245, 240]},
                    },
                    "label": {
                        "text": fix[2],
                        "font": "9px sans-serif",
    ...
    def get_era5_wind(
        ...
        if (
            Path(__file__)
            .parent.parent.joinpath("data/weather/era5/", file.split("-", 1)[0])
            .is_dir()
        ):
            data = xr.open_dataset(
                Path(__file__).parent.parent.joinpath(
                    "data/weather/era5/",
                    file.split("-", 1)[0] + "/multilevel.nc",
                )
            ).sel(level=900, time=datetime.fromisoformat(time), method="pad")
            data = data.where(
                (
                    ((data.latitude >= lat1) & (data.latitude <= lat2))
                    & (
                        (data.longitude >= (long1 + 360.0) % 360.0)
                        & (data.longitude <= (long2 + 360.0) % 360.0)
                    )
                ),
                drop=True,
            )
            fig = Figure(
                figsize=(long2 - long1, lat2 - lat1), facecolor="none", dpi=500
            )
            ax = fig.add_axes(
                [0, 0, 1, 1], projection=ccrs.PlateCarree(), frameon=False
            )  # type: ignore
    ...
    def get_era5_rain(
        ...
        data = xr.open_dataset(
            Path(__file__).parent.parent.joinpath(
                "data/weather/era5/", file.split("-", 1)[0] + "/surface.nc"
            )
        ).sel(time=datetime.fromisoformat(time), method="pad")
        data = data.where(
            (
                ((data.latitude >= lat1) & (data.latitude <= lat2))
                & (
        ...
        ax.set_extent([long1, long2, lat1, lat2], crs=ccrs.PlateCarree())  # type: ignore
        colorscale = [
            "#ffffff00",
            "#00c9fc",
            "#008ff4",
    ...
    def get_radar_img(
        ...
        )  # Minimum value of rainfall rate category
        rain_fall_r = np.array(
            [
                0,
                0,
                60,
        ...
        if not (
            ...
            .parent.parent.joinpath(
                "data/weather/radar/", file.split("-", 1)[0]
            )
            .is_dir()
        ):
            return []
        for fp in (
            ...
            if (
                datetime.fromisoformat(time + "+00:00")
                - datetime.fromisoformat(fp.stem + "+00:00")
            ).seconds > 3600:
                return []
            img = Image.open(fp, "r")
            ...
            distant = [
                np.square(data[:, :, 0].flatten() - rain_fall_r[i])
                + np.square(data[:, :, 1].flatten() - rain_fall_g[i])
                + np.square(data[:, :, 2].flatten() - rain_fall_b[i])
                for i in range(len(rain_fall))
            ]
            distant = np.stack(distant, axis=-1)
            # find index of minimum element row-wise in distance array
            # and grab rain_fall category
            index = np.argmin(distant, axis=1)
            result = np.take(rain_fall, index)
            # Check if the minimum distance < confidence interval
            CONFIDENCE = 6000.0
