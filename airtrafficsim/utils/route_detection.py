    ...
    for ats, waypoints in procedure_dict.items():
        ...
        for i in range(len(simplified_trajectory)-1):
            ...
            if (simplified_trajectory[i][0] >= np.min(wp_lat)) & (simplified_trajectory[i][0] <= np.max(wp_lat)) & (simplified_trajectory[i][1] >= np.min(wp_long)) & (simplified_trajectory[i][1] <= np.max(wp_long)):
                ...
                cross_dist = Cal.cal_cross_track_dist(simplified_trajectory[i][0], simplified_trajectory[i][1], simplified_trajectory[i + 1][0], simplified_trajectory[i + 1][1], wp_list[:, 0], wp_list[:, 1])
                area = np.abs(Cal.cal_great_circle_dist(simplified_trajectory[i][0], simplified_trajectory[i][1], simplified_trajectory[i + 1][0], simplified_trajectory[i + 1][1]) * cross_dist / 2.0)
