...
class Aircraft:
    ...
    def set_vectoring(self, vectoring_time, v_2, fix):
        ...
        if not self.vectoring == fix and self.get_next_wp() == fix:
            ...
            new_dist = self.traffic.ap.dist[index] + Unit.kts2mps(self.traffic.cas[index] + v_2) * (vectoring_time) / 2000.0
            bearing = np.mod(self.traffic.ap.heading[index]+np.rad2deg(np.arccos(self.traffic.ap.dist[index]/new_dist)) + 360.0, 360.0)
            lat, long = Cal.cal_dest_given_dist_bearing(self.traffic.lat[index], self.traffic.long[index], bearing, new_dist / 2)
