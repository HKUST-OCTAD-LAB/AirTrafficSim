...
class Autopilot:
    ...
    def add_aircraft(
        ...
        self.auto_throttle_mode = np.append(
            self.auto_throttle_mode, APThrottleMode.SPEED
        )
        ...
        if not flight_plan == []:
            ...
            if cruise_alt > -1:
                self.flight_plan_target_alt[-1].extend(
                    [cruise_alt for _ in flight_plan]
                )
        ...
        if not star == "":
            (
                ...
                speed_resctriction_type,
                speed_restriction,
            ) = Nav.get_procedure(arrival_airport, arrival_runway, star)
            self.flight_plan_name[-1].extend(waypoint)
            self.flight_plan_target_alt[-1].extend(alt_restriction)
            self.flight_plan_target_speed[-1].extend(speed_restriction)
        ...
        if not approach == "":
            ...
            (
                ...
                arrival_airport, arrival_runway, approach, appch="I"
            )
            # Remove last element of flight plan which should be equal to iaf
            self.flight_plan_name[-1].pop()
            self.flight_plan_target_alt[-1].pop()
        ...
        for i, val in enumerate(self.flight_plan_name[-1]):
            if i == 0:
                ...
                lat_tmp, long_tmp = Nav.get_wp_coord(
                    val,
                    self.flight_plan_lat[-1][i - 1],
                    self.flight_plan_long[-1][i - 1],
                )
                self.flight_plan_lat[-1].append(lat_tmp)
                self.flight_plan_long[-1].append(long_tmp)
        ...
        if not arrival_runway == "":
            lat_tmp, long_tmp, alt_tmp = Nav.get_runway_coord(
                arrival_airport, arrival_runway
            )
    ...
    def update(self, traffic: Traffic) -> None:
        ...
        self.procedure_speed = traffic.perf.get_procedure_speed(
            traffic.alt, traffic.trans_alt, traffic.configuration
        )
        # TODO: Check procedure speed with SID STAR limitation

        self.cas = np.where(
            (self.auto_throttle_mode == APThrottleMode.AUTO)
            & (self.procedure_speed >= 5.0),
            self.procedure_speed,
            self.cas,
        )  # TODO: Add speed mode atc
        ...
        self.mach = np.where(
            traffic.speed_mode == SpeedMode.CAS,
            traffic.perf.tas_to_mach(
                traffic.perf.cas_to_tas(
                    Unit.kts2mps(self.cas),
                    traffic.weather.p,
                    traffic.weather.rho,
                ),
                traffic.weather.T,
        ...
        self.cas = np.where(
            traffic.speed_mode == SpeedMode.MACH,
            Unit.mps2kts(
                traffic.perf.tas_to_cas(
                    traffic.perf.mach_to_tas(self.mach, traffic.weather.T),
                    traffic.weather.p,
                    traffic.weather.rho,
                )
            ),
        ...
        # Vertical mode
        traffic.vertical_mode = np.select(
            condlist=[
        ...
        # Fly by turn
        turn_radius = (
            traffic.perf.cal_turn_radius(
                traffic.perf.get_bank_angles(traffic.configuration),
                Unit.kts2mps(traffic.tas),
            )
            / 1000.0
        )  # km
        next_track_angle = np.where(
            self.hv_next_wp,
            Cal.cal_great_circle_bearing(
                self.lat, self.long, self.lat_next, self.long_next
            ),
            self.track_angle,
        ...
        self.track_angle = np.where(
            self.lateral_mode == APLateralMode.HEADING,
            0.0,
            np.where(
                dist < turn_dist,
        ...
        self.heading = np.where(
            ...
            self.track_angle
            + np.arcsin(
                traffic.weather.wind_speed
                / traffic.tas
                * np.sin(self.track_angle - traffic.weather.wind_direction)
            ),
        )  # https://www.omnicalculator.com/physics/wind-correction-angle

        update_next_wp = (
            (self.lateral_mode == APLateralMode.LNAV)
            & (dist > self.dist)
            & (
                np.abs(Cal.cal_angle_diff(traffic.heading, next_track_angle))
                < 1.0
            )
        )
        self.flight_plan_index = np.where(
            update_next_wp, self.flight_plan_index + 1, self.flight_plan_index
        )
        self.dist = np.where(
            update_next_wp,
            Cal.cal_great_circle_dist(
                traffic.lat, traffic.long, self.lat_next, self.long_next
            ),
            dist,
        )
        ...
        for i, val in enumerate(self.holding):
            if self.holding[i] is False:
                if (
                    ...
                    < 90.0
                    and self.flight_plan_index[i]
                    > self.flight_plan_name[i].index(self.holding_info[i][0])
                ):  # Turn outbound
                ...
                if (
                    np.abs(
                        Cal.cal_angle_diff(
                            self.heading[i], self.holding_info[i][4] + 180.0
                        )
                    )
                    < 90.0
                    and dist[i] > Unit.nm2m(self.holding_info[i][6]) / 1000.0
                ):  # Turn inbound
                    self.heading[i] = self.holding_info[i][4]
