...
class Nav:
    ...
    if (
        ...
        .joinpath("./data/navigation/xplane/")
        .is_dir()
    ):
        # Create directories
        Path(__file__).parent.parent.resolve().joinpath(
            "./data/navigation/xplane/airports"
        ).mkdir(parents=True)
        ...
        ZipFile(
            Path(__file__)
            .parent.parent.resolve()
            .joinpath("./data/navigation/xplane_default_data.zip")
        ).extractall(
            Path(__file__)
            .parent.parent.resolve()
            .joinpath("./data/navigation/xplane/")
        ...
        with open(
            Path(__file__)
            .parent.parent.resolve()
            .joinpath("./data/navigation/xplane/apt.dat"),
            "r",
        ) as file:
            # Skip 3 lines
            next(file)
                ...
                if row:
                    ...
                    if row[0] in ("1", "16", "17", "99"):
                        ...
                        if not icao == "":
                            print(
                                "\r"
                                + "Extracting information from airport "
                                + icao,
                                end="",
                                flush=True,
                            )
    ...
    fix = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_fix.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
    )
    ...
    nav = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_nav.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
        names=np.arange(0, 18),
        low_memory=False,
    ...
    airway = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_awy.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
    )
    ...
    holding = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_hold.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
    )
    """Holding data https://developer.x-plane.com/wp-content/uploads/2018/12/XP-HOLD1140-Spec.pdf"""
    min_off_route_alt = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_mora.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
    )
    """Minimum off route grid altitudes https://developer.x-plane.com/wp-content/uploads/2020/03/XP-MORA1150-Spec.pdf"""
    min_sector_alt = pd.read_csv(
        Path(__file__)
        .parent.parent.resolve()
        .joinpath("./data/navigation/xplane/earth_msa.dat"),
        delimiter=r"\s+",
        skiprows=3,
        header=None,
    ...
    def get_wp_in_area(
        ...
        if lat1 < lat2 and long1 < long2:
            ...
            fix = (
                Nav.fix[
                    (Nav.fix.iloc[:, 0].between(lat1, lat2))
                    & (Nav.fix.iloc[:, 1].between(long1, long2))
                ]
                .iloc[:, 0:3]
                .to_numpy()
            )
            nav = (
                Nav.nav[
                    (Nav.nav.iloc[:, 1].between(lat1, lat2))
                    & (Nav.nav.iloc[:, 2].between(long1, long2))
                ]
                .iloc[:, [1, 2, 7]]
                .to_numpy()
            )
            ...
            fix = (
                Nav.fix[
                    (Nav.fix.iloc[:, 0].between(lat1, lat2))
                    & (
                        Nav.fix.iloc[:, 1].between(long1, 180.0)
                        | Nav.fix.iloc[:, 1].between(-180.0, long2)
                    )
                ]
                .iloc[:, 0:3]
                .to_numpy()
            ...
            nav = (
                Nav.nav[
                    (Nav.nav.iloc[:, 1].between(lat1, lat2))
                    & (
                        Nav.nav.iloc[:, 2].between(long1, 180.0)
                        | Nav.nav.iloc[:, 2].between(-180.0, long2)
                    )
                ]
                .iloc[:, [1, 2, 7]]
                .to_numpy()
            ...
            fix = (
                Nav.fix[
                    (
                        Nav.fix.iloc[:, 0].between(lat1, 90.0)
                        | Nav.fix.iloc[:, 0].between(lat1, -90.0)
                    )
                    & (Nav.fix.iloc[:, 1].between(long1, long2))
                ]
            ...
            nav = (
                Nav.nav[
                    (
                        Nav.nav.iloc[:, 1].between(lat1, 90.0)
                        | Nav.nav.iloc[:, 1].between(lat1, -90.0)
                    )
                    & (Nav.nav.iloc[:, 2].between(long1, long2))
                ]
    ...
    def find_closest_airport_runway(lat: float, long: float) -> tuple[str, str]:
        ...
        """
        tmp = Nav.airports[
            (Nav.airports.iloc[:, 2].between(lat - 0.1, lat + 0.1))
    ...
    def get_procedure(
        ...
        if appch == "":
            ...
            # Final Approach
            procedure_df = procedures[
                (procedures[1] == appch) & (procedures[2] == procedure)
            ]
        ...
        alt_restriction = np.where(
            np.array(alt_restriction_2) != -1,
            np.minimum(alt_restriction_1, alt_restriction_2),
            alt_restriction_1,
        )
