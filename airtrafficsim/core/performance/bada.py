...
class Bada:
    ...
    def __init__(self):
        ...
        if Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/BADA.GPF').is_file():
            GPF = np.genfromtxt(Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/BADA.GPF'), delimiter=[3,16,8,17,29,12], dtype="U2,U15,U7,U16,U29,f8", comments="CC", autostrip=True, skip_footer=1)
        ...
        self.__SYNONYM = np.genfromtxt(Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/SYNONYM.NEW'), delimiter=[3,2,7,20,25,8,5], names=['CD','ST','ACCODE','MANUFACTURER','MODEL','FILENAME','ICAO'], dtype="U2,U1,U4,U18,U25,U6,U1", comments="CC", autostrip=True, skip_footer=1, encoding= 'unicode_escape')
    ...
    def add_aircraft(self, icao, mass_class=2):
        ...
        OPF = np.genfromtxt(Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/', file_name+'.OPF'), delimiter=[3,2,2,13,13,13,13,11], dtype="U2,U1,U2,f8,f8,f8,f8,f8", comments="CC", autostrip=True, skip_header=16, skip_footer=1)
        ...
        OPF_Actype = np.genfromtxt(Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/', file_name+'.OPF'), delimiter=[5,15,1,12,26,1], dtype="U2,U6,i1,U7,U9,U1", comments="CC", autostrip=True, max_rows=1)
        ...
        APF = np.genfromtxt(Path(__file__).parent.parent.parent.parent.resolve().joinpath('./data/performance/BADA/', file_name+'.APF'), delimiter=[6,8,9,4,4,4,3,5,4,4,4,4,3,4,4,5,4,4,4,5,7], dtype="U2,U7,U7,U2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,i2,U6", comments="CC", autostrip=True)
    ...
    def cal_thrust(self, vertical_mode, configuration, H_p, V_tas, d_T, drag, ap_speed_mode):
        ...
        return np.select(
                    condlist=[
                        (vertical_mode == VerticalMode.CLIMB) | ((vertical_mode == VerticalMode.LEVEL) & (ap_speed_mode == APSpeedMode.ACCELERATE)),
                        (vertical_mode == VerticalMode.LEVEL) & ((ap_speed_mode == APSpeedMode.CONSTANT_CAS) | (ap_speed_mode == APSpeedMode.CONSTANT_MACH)),
    ...
    def init_procedure_speed(self, m, n):
        ...
        v_stall_to_act = Unit.mps2kts(self.__cal_operating_speed(m, Unit.kts2mps(self.__v_stall_to))[n])
        ...
        v_stall_ld_act = Unit.mps2kts(self.__cal_operating_speed(m, Unit.kts2mps(self.__v_stall_ld))[n])
    ...
    def update_configuration(self, V_cas, H_p, vertical_mode):
        ...
        return np.select(condlist=[
            ...
            (vertical_mode == VerticalMode.CLIMB) & (H_p > self.__H_MAX_TO) & (H_p < self.__H_MAX_IC),
            (H_p > self.__H_MAX_IC) | (vertical_mode == VerticalMode.DESCENT) & ((V_cas >= (self.__C_V_MIN * self.__v_stall_cr + 10.0))),
            (vertical_mode == VerticalMode.DESCENT) & ((V_cas < self.__C_V_MIN * self.__v_stall_cr + 10.0) & (H_p > self.__H_MAX_LD) & (H_p <= self.__H_MAX_AP)) | ((V_cas < (self.__C_V_MIN * self.__v_stall_cr + 10.0)) & (V_cas >= (self.__C_V_MIN * self.__v_stall_ap + 10.0)) & (H_p <= self.__H_MAX_LD)),
            (vertical_mode == VerticalMode.DESCENT) & (H_p < self.__H_MAX_LD) & (V_cas < (self.__C_V_MIN * self.__v_stall_ap + 10.0))
                        ],
                         choicelist=[
                             Config.TAKEOFF,
