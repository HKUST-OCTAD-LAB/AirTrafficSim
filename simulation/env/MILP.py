...
class MILP(Environment):
    ...
    def __init__(self):
        ...
        # Set up arrival data
        arrival_procedures = Nav.get_instrument_procedures("VHHH", "STAR")
        ...
        for wp in arrival_waypoints:
            coord = Nav.get_fix_coordinate(wp, 22.3080, 113.9185)
            arrival_waypoints_coord_dict[wp] = list(coord)

        # Set up approach data
        approach_procedures = Nav.get_instrument_procedures("VHHH", "APPCH")
        ...
        for file in Path(__file__).parent.parent.parent.resolve().joinpath('data/replay/historic/2018-05-01/').iterdir():
